...
const DataTable: React.FunctionComponent<IDataTableProps> = (props) => {
    ...
    return data.map((item) => {
      return (
        ...
        >
          ...
          <td className="px-6 py-3 break-words">
            <Link
              to={`${servUrl}/shortUrl/${item.shortUrl}`}
              target="_blank"
  ...
  const copyToClipboard = async (url: string) => {
    try {
      await navigator.clipboard.writeText(`${servUrl}/shortUrl/${url}`);
      alert(`Copied to Clipboard,${servUrl}/shortUrl/${url}`);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const deleteUrl = async (id: string) => {
    const response = await axios.delete(`${servUrl}/shortUrl/${id}`);
    updateReload();
